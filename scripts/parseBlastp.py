...
sequence length between 0.5*len(query) - 1.5*len(query) and identity to query > 35%.
Usage:
            ...
            for hsp in alignment.hsps:
                ...
                if 0.5 < float(hit_end - hit_start) / query_length < 2.0:
                    if float(hsp.identities)/query_length > 0.35:
                        ...
                        idList.append((hit_id, hit_start, hit_end, hit_length))
    return(idList)

def retrieveFasta(idList):
    #print(idList)
    fastaList = []
    for id_tuple in idList:
        ...
        efetch_command = "efetch -id " + id + " -db protein -format fasta"
        loginfo = efetch_command
        proc_efetch = subprocess.Popen(efetch_command, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        out, err = proc_efetch.communicate()
        out = out.decode()
        nameline, seqlines = out.split("\n")[0], ''.join(out.split('\n')[1:])
        nameline = nameline + '\t' + str(int(hit_length)) + ':' + str(hit_start) + ':' + str(hit_end)
        seqline = seqlines[hit_start - 1:hit_end]
...
def main():
    ...
    fastaList = retrieveFasta(idList)
    #print(fastaList)
    outfile = open(''.join([str(work_dir), '/Blastp_fasta/', filename.split("/")[-1], '.fasta']), 'w+')
    outfile.write(''.join(fastaList))
    outfile.close()
    loginfo = (" ".join([filename, ":", str(len(fastaList)), 'filtered blastp hits saved to Blastp_fasta folder.']))
    print(loginfo)
